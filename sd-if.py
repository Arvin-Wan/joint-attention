...
def createModel(input_data, input_size, sequence_length, slots, slot_size, intent_size, layer_size=128,
    ...
    # state_outputs 是双向lstm的输出，正向的和反向的输出； 而final state 是双向lstm的 c 和 h，一共有2个c 和 2 个h。
    # state_outputs : 
        # fw output : shape=(B, S, D)
        # bw output : shape=(B, S, D)
    # Final state:
        # fw: 
            # c shape= (B, D)
            # h shape= (B,D)
    ...
    final_state = tf.concat([final_state[0][0], final_state[0][1], final_state[1][0], final_state[1][1]], 1)       # concat([c,h,c,h]) shape=(B, 4D)
    state_outputs = tf.concat([state_outputs[0], state_outputs[1]], 2)      # shape = (B, S, 2D)
    state_shape = state_outputs.get_shape()        # B,  S, 2D

    with tf.variable_scope('attention'):
        slot_inputs = state_outputs
        if not remove_slot_attn:
            with tf.variable_scope('slot_attn'):
                attn_size = state_shape[2].value                         # attn size = 2D
                origin_shape = tf.shape(state_outputs)
                hidden = tf.expand_dims(state_outputs, 1)                #B, 1, S, 2D
                hidden_conv = tf.expand_dims(state_outputs, 2)            # hidden_conv = state_outputs      shape=(B, S, 1, 2D)
                k = tf.get_variable("AttnW", [1, 1, attn_size, attn_size])      # shape = (1,1, 2D, 2D)
                hidden_features = tf.nn.conv2d(hidden_conv, k, [1, 1, 1, 1], "SAME")   # hidden_features = conv(hidden_conv)
                hidden_features = tf.reshape(hidden_features, origin_shape)      
                hidden_features = tf.expand_dims(hidden_features, 1)                # shape=(B,1, S ,2D)
                v = tf.get_variable("AttnV", [attn_size])                   # shape=(2D)
                slot_inputs_shape = tf.shape(slot_inputs)
                slot_inputs = tf.reshape(slot_inputs, [-1, attn_size])    # B *s, 2D
                y = core_rnn_cell._linear(slot_inputs, attn_size, True)    #y =  W * C_{slot},     slot_inputs = C_{slot}  =  state_inputs
                y = tf.reshape(y, slot_inputs_shape)
                y = tf.expand_dims(y, 2)                                 # B, S, 1, 2D
                s = tf.reduce_sum(v * tf.tanh(hidden_features + y), [3])     # f = \sum V *tanh(C_{slot} + y)     B, S, S
                a = tf.nn.softmax(s)                                         # a = softmax(f)        B, S, S
                a = tf.expand_dims(a, -1)                                    # B, S, S, 1
                slot_d = tf.reduce_sum(a * hidden, [2])                      # B, S, 2D
                slot_reinforce_state = tf.expand_dims(slot_d, 2)             # B, S, 1, 2D
        else:
        ...
        with tf.variable_scope('intent_attn'):
            ...
            hidden_features = tf.nn.conv2d(hidden, k, [1, 1, 1, 1], "SAME")   # hidden_state = conv(state_outputs)
            v = tf.get_variable("AttnV", [attn_size])

            y = core_rnn_cell._linear(intent_input, attn_size, True)          # y = W * h
            y = tf.reshape(y, [-1, 1, 1, attn_size])
            s = tf.reduce_sum(v * tf.tanh(hidden_features + y), [2, 3])       # s =  V * tanh(hidden_features + y)
            a = tf.nn.softmax(s)                                              # a = softmax(s)
            a = tf.expand_dims(a, -1)
            a = tf.expand_dims(a, -1)
            d = tf.reduce_sum(a * hidden, [1, 2])                             # r_inte = a * state_outputs
            r_intent = d
        ...
        if arg.priority_order == 'intent_first':
            ...
            for n in range(arg.iteration_num):
                with tf.variable_scope('slot_subnet' + str(n - 1)):
                    ...
                    relation_factor = v1 * tf.tanh(slot_d + intent_gate)         
                    relation_factor = tf.reduce_sum(relation_factor, [2])
                    relation_factor = tf.expand_dims(relation_factor, -1)                          # f = tanh(C_slot + C_inte)
                    slot_reinforce_state = slot_d * relation_factor                                # r_{slot} = C_{slot} * f
                    slot_reinforce_vector = tf.reshape(slot_reinforce_state, [-1, attn_size])
                    slot_output = tf.concat([slot_reinforce_vector, slot_inputs], 1)               # concat(h, r_slot)

                with tf.variable_scope('intent_subnet' + str(n - 1)):
                    attn_size = state_shape[2].value
                    hidden = tf.expand_dims(state_outputs, 2)                                        # h  也就是token embedding
                    slot_reinforce_output = tf.expand_dims(slot_reinforce_state, 2)                  # r_{slot}
                    k1 = tf.get_variable("W1", [1, 1, attn_size, attn_size])
                    k2 = tf.get_variable('W2', [1, 1, attn_size, attn_size])
                    slot_features = tf.nn.conv2d(slot_reinforce_output, k1, [1, 1, 1, 1], "SAME")    # slot_feature = conv2(r_{slot})
                    hidden_features = tf.nn.conv2d(hidden, k2, [1, 1, 1, 1], "SAME")                 #h_features = conv(h)
                    v1 = tf.get_variable("AttnV", [attn_size])
                    bias = tf.get_variable("Bias", [attn_size])
                    s = tf.reduce_sum(v1 * tf.tanh(hidden_features + slot_features + bias), [2, 3])    # e = V1 * tanh(h_feature + slot_feature + b)
                    a = tf.nn.softmax(s)                                                        
                    a = tf.expand_dims(a, -1)
                    a = tf.expand_dims(a, -1)                                                        # a = softmax(e)
                    r = tf.reduce_sum(a * slot_reinforce_output, [1, 2])                             # r = a * r_{slot}

                    r_intent = r + intent_context_states

                    intent_output = tf.concat([r_intent, intent_input], 1)                           # concat(r_inte, final_state)
