...
class Encoder(nn.Module):
    def __init__(self, p_dropout=0.5,patch_size=32):
        ...
        self.filter_number = ENV_EMBEDDING_SIZE // 4
        self.kernel_number = ENV_CNN_KERNELS  # tedad size haye filter : 2,3,5 = 3
        self.embedding_size = ENV_EMBEDDING_SIZE
        # self.activation = nn.ReLU()
        self.activation2 = RMSNorm(self.embedding_size)
        self.activation = nn.LeakyReLU()
        self.softmax = nn.Softmax(dim=1)
        self.linear1 = nn.Linear(self.embedding_size, self.embedding_size)
        ...
        self.tanh_linear = nn.Linear(self.embedding_size, self.embedding_size // 4)
        self.tanh_activation = nn.Tanh()
        self.softmax_activation =nn.Sigmoid()
        # nn.Softmax(-1)
    ...
    def forward(self, bert_last_hidden,bert_pooler_output=None,isid=False):
        ...
        y = torch.cat((convolve1, convolve2,convolve3,convolve4), dim=2)
        y = torch.unsqueeze(y, 1)

        linear_output = self.activation2(self.dropout(self.linear1(bert_last_hidden)))
        linear_output = torch.unsqueeze(linear_output, 2)

        # output = torch.sum(self.V * self.tanh_activation(self.tanh_linear(output)),dim = -1)
        output = torch.mean(self.tanh_activation(y + linear_output), dim = -1)
        # if isid:
        #     output += bert_pooler_output
        output = self.softmax_activation(output)
        output = torch.unsqueeze(output, -1)
        output = output * bert_last_hidden.unsqueeze(1)
...
class Decoder(nn.Module):
    ...
    def __init__(self,slot_size,intent_size,dropout_p=0.7,ratio_squeeze= 4):
        ...
        self.id_attn = SENET(ENV_EMBEDDING_SIZE)
        ...
        self.cross_id_ln = nn.LayerNorm(ENV_EMBEDDING_SIZE)
        self.cross_sf_ln = nn.LayerNorm(ENV_EMBEDDING_SIZE)



    def forward(self, input,bert_outputs,encoder_maskings,bert_subtoken_maskings=None,infer=False,bert_pooler_outputs=None):
        id_attn = self.id_attn(bert_outputs)
        sf_attn = self.sf_attn(bert_outputs)

        # print(id_attn.shape, sf_attn.shape)
        ...
        # cross_attn,_ = self.self_attention(cross_attn,cross_attn,cross_attn,key_padding_mask=encoder_maskings)
        ...
        cross_sf = cross_attn_sf + sf_attn

        cross_id = self.cross_id_ln(cross_id)
        cross_sf = self.cross_sf_ln(cross_sf)
        ...
        intent_scores = self.intent_out(self.dropout1(sum_encoder/sum_mask + bert_pooler_outputs)) # B,D

        # cross_id=torch.transpose(sum_encoder,dim0=1,dim1=2)
